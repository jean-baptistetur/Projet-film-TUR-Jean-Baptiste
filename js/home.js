class HomePage {
    ...
    async loadTrendingMovies() {
        try {
            ...
            this.trendingGrid.innerHTML = response.results.map(m => this.buildCard(m)).join("");
        } catch (error) {
    ...
    async loadFilms() {
        try {
            const response = await api.getPopularMovies(this.filmsGenreSelect.value, this.filmsYearSelect.value);
    ...
    async loadSeries() {
        try {
            const response = await api.getPopularSeries(this.seriesGenreSelect.value, this.seriesYearSelect.value);
